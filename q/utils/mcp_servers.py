...
This module handles loading MCP server configurations from both the built-in
constants and the user's custom configuration file at ~/.config/q/mcp-servers.json.
"""
...
from q.core.constants import MCP_SERVERS as DEFAULT_MCP_SERVERS
...
def get_all_mcp_servers() -> Dict[str, Dict[str, Any]]:
    """
    Get all available MCP servers by combining default and user-defined servers.
    
    User-defined servers will override default servers with the same name.
    
    Returns:
        Dictionary mapping server names to their configuration details
    """
    # Start with the default servers from constants
    all_servers = DEFAULT_MCP_SERVERS.copy()
    
    # Load user-defined servers if available
    user_servers, error_msg = load_user_mcp_servers()
    
    # Merge user servers, allowing them to override defaults with the same name
    if user_servers:
